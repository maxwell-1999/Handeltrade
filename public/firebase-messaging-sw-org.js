...
self.addEventListener('push', function (event) {
  ...
  request.onsuccess = function (event) {
    ...
    store.get(data.id).onsuccess = function (event) {
      ...
      console.log("Data: ", event.target.result.value);
  ...
  messaging.onBackgroundMessage((payload) => {
    ...
    request.onsuccess = function (event) {
      ...
      store.get("is_popup_on").onsuccess = function (event) {
        ...
        console.log("Notification is for topic:", topic);

        console.log("Is popup on: ", event.target.result);
        if (event.target.result && event.target.result.value && payload.data) {
          store = db.transaction("configs", "readonly").objectStore("configs");
          console.log("Notification Again is for topic:", topic);
          store.get(topic).onsuccess = function (event) {
            console.log("Notification is on:: ", topic, event.target.result.value);
